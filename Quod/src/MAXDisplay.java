...
public class MAXDisplay extends JInternalFrame implements ActionListener {
   ...
   */
  public MAXDisplay(MAXPlayer[] player, MAXBoard board) {
    super("MAX", true, true);
    this.player = player;
    this.board = board;
   ...
   * @param checkScore - Boolean if score limit is reached
   */
  ...
  /**
   * Displays the formated board on the screen. It shows its values and spots the players and then
   * assigns them with a predefined letter or generic name.
   * 
   * @param board - The game board
   */
  private void showBoard(MAXBoard board) {
    int i = 0;
  ...
  /**
   * Event listener for running the game logic. It contains the movement and score- and board
   * display.
   * 
   */
  @Override
  public void actionPerformed(ActionEvent e) {
    ...
    MAXAction action = MAXAction.of(movement.toLowerCase().substring(0, 1));
    // temporary variable to hold current player
    MAXPlayer currentPlayer = player[i];
   ...
   * @param game
   */
  public void setGame(MAXGame game) {
    this.game = game;
  }

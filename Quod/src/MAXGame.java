...
public class MAXGame {
  ...
  private MAXPlayer[] player;
  private MAXBoard board;
  private MAXBoard clonedBoard;
  private MAXPlayerPosition playerPosition;
  private MAXDisplay display;
  private boolean restart;

  /**
   * The constructor of the game.
   * 
   * @param boardSizeX - X size of the board
   * @param boardSizeY - Y size of the board
   * @param playerCount - Amount of players
   */
  public MAXGame(int boardSizeX, int boardSizeY, int playerCount) {

    this.BOARD_SIZE_X = boardSizeX;
    this.BOARD_SIZE_Y = boardSizeY;
    this.PLAYER_COUNT = playerCount;
    this.player = new MAXPlayer[playerCount];
    this.restart = false;
   ...
   * @param direction
   */
  public void move(MAXPlayer currentPlayer, MAXAction direction) {
  ...
  private void initializeGame() throws CloneNotSupportedException {
    ...
    playerPosition = new MAXPlayerPosition(BOARD_SIZE_X, BOARD_SIZE_Y, PLAYER_COUNT);
    int[][] position = playerPosition.getPosition();
    // create players with starting location and color
    for (int i = 0; i < PLAYER_COUNT; i++) {
      player[i] = new MAXPlayer(position[i][0], position[i][1], (i + 1) * (-1));
    }
    try {
      // restart game logic
      if (restart) {
        ...
        board = new MAXBoard(BOARD_SIZE_X, BOARD_SIZE_Y);
        clonedBoard = board.clone();
      }
    } catch (CloneNotSupportedException e) {
      MAXIO.writeln("error in cloning");
    }
    ...
    this.display = new MAXDisplay(player, board);
    display.setGame(this);
    display.draw(-1, checkScore());
  }

  /**
   * Gets the old position of the player and sets the value inside the board to 0.
   * 
   * @param player - Which player position should be reset to 0
   */
  private void removePlayerFromPreviousPosition(MAXPlayer player) {
   ...
   * 
   * @param player - Which player should be tested
   * @param direction - Which direction does the player want to go
   * @return - Returns boolean true if move is legitimate
   */
  private boolean isOutOfBounds(MAXPlayer player, MAXAction direction) {
    switch (direction) {
      case LEFT:
   ...
   * @return - Returns boolean true if move is legitimate
   */
  private boolean isColliding(MAXPlayer player, MAXAction direction) {
    int x = player.getX();
    int y = player.getY();
    switch (direction) {
      case LEFT:
   ...
   * @return - Returns boolean true if move is legitimate
   */
  public boolean canMoveInDirection(MAXPlayer player, MAXAction direction) {
   ...
   * 
   * @param playerID - ID of the wrongly assumed player.
   ...
   * 
   * @param currentPlayer - player whos turn it is
   */
  private void addScoreAndSetNewPosition(MAXPlayer currentPlayer) {
